...
Point drawBezierGeneralized(Point PT[], double t)
{
    ...
    {
        P.x = P.x + binomial_coff((float)(clicks - 1),
                                  (float)i) *
                        pow(t, (double)i) * pow((1 - t), (clicks - 1 - i)) * PT[i].x;
        P.y = P.y + binomial_coff((float)(clicks - 1),
                                  (float)i) *
                        pow(t, (double)i) * pow((1 - t), (clicks - 1 - i)) * PT[i].y;
    }
    // cout<<P.x<<endl<<P.y;
    // cout<<endl<<endl;
    return P;
...
void myMouse(int button, int state, int x, int y)
{
                                          ...
                                          GLUT_DOWN)
    {
        ...
        abc[points].setxy((float)x, (float)(SCREEN_HEIGHT -
                                            y));
        points++;
        // Draw the red dot.
        ...
        if (points == clicks)
        {
            ...
            Point p1 = abc[0];
            /* Draw each segment of the curve.Make t
            increment in smaller amounts for a more detailed
            curve.*/
